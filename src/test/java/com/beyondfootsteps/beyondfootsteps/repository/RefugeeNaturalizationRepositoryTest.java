...
@DataJpaTest
@ActiveProfiles("test")
class RefugeeNaturalizationRepositoryTest {
    ...
    void shouldSaveAndFindByYearAndCountries(int year, String originCountry, String asylumCountry, int yearToLookFor, String countryOriginToLookFor, String countryAsylumToLookFor, int resultsExpected) {
        refugeeNaturalizationRepository.deleteAll();
        RefugeeNaturalization naturalization = new RefugeeNaturalization();
        naturalization.setId("test-id");
        naturalization.setYear(year);
        naturalization.setCountryOfOriginIso(originCountry);
        naturalization.setCountryOfAsylumIso(asylumCountry);

        refugeeNaturalizationRepository.save(naturalization);

        List<RefugeeNaturalization> results = refugeeNaturalizationRepository.findByYearAndCountries(yearToLookFor, countryOriginToLookFor, countryAsylumToLookFor);
